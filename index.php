...
if (!$res_new || $xoopsDB->getRowsNum($res_new) < 1) {
    ...
    for ($i = 0; $i < $rows; $i++) {
        ...
        if (!($G_RELEASE[(string)$row_new[group_id]])) {
            print '<TR valign=top>';

            print '<TD colspan=2>';

            print "<A href='" . XOOPS_URL . '/modules/xfmod/project/?' . $row_new['unix_group_name'] . "'><B>" . $ts->htmlSpecialChars($row_new['group_name']) . '</B></A>'
                   . "\n</TD><TD nowrap><I>" . _XF_NEW_RELEASEDBY . ": <A href='" . XOOPS_URL . "/userinfo.php?uid=$row_new[uid]/'>"
                   . "$row_new[uname]</A></I></TD></TR>\n";

            print '<TR><TD>' . _XF_NEW_MODULE . ': ' . $ts->htmlSpecialChars($row_new['module_name']) . "</TD>\n";
            ...
            if ($row_new['short_description']) {
                print '<I>' . $ts->displayTarea($row_new['short_description']) . '</I>';
            } else {
            ...
            $G_RELEASE[(string)$row_new[group_id]] = 1;
        }
    }
